...
abstract class PluginPhotos extends BasePhotos
{
    ...
    {
        $path = '/uploads/gallery/'.$this->getGallery()->getId()."/";
        if ($size) {
            $path .= $size."/";
        }
    ...
    public function save(Doctrine_Connection $conn = null)
    {
        ...
        if(file_exists(sfConfig::get("app_sfMultipleAjaxUploadGalleryPlugin_path_gallery")."tmp/".$filename)){
            copy (
                    ...
                    sfConfig::get("app_sfMultipleAjaxUploadGalleryPlugin_path_gallery").$this->getGallery()->getId()."/".$filename );
//            unlink(sfConfig::get("app_sfMultipleAjaxUploadGalleryPlugin_path_gallery")."tmp/".$filename);
    ...
    {
        $img = new sfImage(sfConfig::get("app_sfMultipleAjaxUploadGalleryPlugin_path_gallery").$this->getGallery()->getId().'/'.$this->getPicpath(), 'image/'.$this->getExtension());
        $w = $img->getWidth();
        $h = $img->getHeight();
        ...
        foreach($sizes as $size) {
            if(is_numeric($size))
            {
                ...
                $img->resize($x,$size);
                $img->setQuality(100);
                $dir = sfConfig::get("app_sfMultipleAjaxUploadGalleryPlugin_path_gallery").$this->getGallery()->getId().'/'.$size;
                if (!is_dir($dir)) {
                    mkdir ($dir);
    ...
    public function  delete(Doctrine_Connection $conn = null)
    {
        ...
        unlink(sfConfig::get("app_sfMultipleAjaxUploadGalleryPlugin_path_gallery").$this->getGallery()->getId().'/'.$filename);
        foreach (sfConfig::get("app_sfMultipleAjaxUploadGalleryPlugin_thumbnails_sizes") as $size) {
            unlink(sfConfig::get("app_sfMultipleAjaxUploadGalleryPlugin_path_gallery").$this->getGallery()->getId().'/'.$size.'/'.$filename);
        }
    }
    ...
    public function overlay($position = 'top-left')
    {
        ...
        $picpath = $uploadDir.$this->getGallery()->getId()."/".$this->getPicpath();
        $float = 45;
